...
#SBATCH --job-name DACORL-HPO-constant
...
mkdir -p jobs/logs/constant_hpo jobs/data/nanoGPT/constant
...
echo "Conda environment activated. Running DACORL Teacher HPO (Constant)..."
python -u teacher_hpo_SGD.py \
  ...
  seed=42 \
  2>&1 | tee -a "jobs/logs/constant_hpo/run-$(date +%Y%m%d-%H%M%S).log"

echo "=========================================="
echo "Job execution complete at $(date)"
echo "Results saved in: data/nanoGPT/constant/"
echo "=========================================="
