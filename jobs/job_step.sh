...
#SBATCH --job-name DACORL-HPO-step
...
mkdir -p jobs/logs/step_hpo jobs/data/nanoGPT/step_decay
...
echo "Conda environment activated. Running DACORL Teacher HPO (Step Decay)..."
python -u teacher_hpo_SGD.py \
  ...
  seed=42 \
  2>&1 | tee -a "jobs/logs/step_hpo/run-$(date +%Y%m%d-%H%M%S).log"

echo "=========================================="
echo "Job execution complete at $(date)"
echo "Results saved in: data/nanoGPT/step_decay/"
echo "=========================================="
